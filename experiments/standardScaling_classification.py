...
if __name__ == '__main__':
    ...
    for entry in models.items():
        ...
        if modelname not in exclude:
            pipelines['Scaled' + modelname] = pipeline.Pipeline([('Scaler', pp.StandardScaler()), (modelname, model(**args))])
    ...
    dfScoresScaled = None
    datasetnames = ['adult']
    datasetnames = ['abalone', 'bankMarketing', 'wineCultivar', 'winequality-red', 'winequality-white', 'iris', 'pima', 'adult']
    for datasetname in datasetnames:
        if 'classification' in datasets[datasetname]['probtype']:
            ...
            if len(xcolsnonnum) > 0:
                ...
                idx = XnonnumOhe.sum(axis = 1) == 0     # find all rows that don't fit in another category
            ...
            for entry in pipelines.items():
                ...
                ppl = entry[1]
                score = -1 * sms.cross_val_score(ppl, X, y, cv = cvsplitter, scoring = scoring)
            ...
            fig.tight_layout()
            fig.savefig('.\\experiments\\standardScaling_classification_' + datasetname + '.png', format = 'png')
            #plt.show()
            plt.close('all')
    ...
    print(dfImprovement)
    with open('.\\experiments\\standardScaling_classification.pkl','wb') as fl:
